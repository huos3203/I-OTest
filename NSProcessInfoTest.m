...
int main(int argc , char * argv[])
{
	@autoreleasepool{
        ...
        //NSLog(@"设置自定义进程名：%@",[proInfo setProcessName:@"nohao"]);
		NSLog(@"进程所在系统的主机名为：%@", [proInfo hostName]);
        //NSLog(@"进程所在系统的操作系统为：%d" , [proInfo operatingSystemVersion]);//[proInfo operatingSystem]);
		//NSLog(@"进程所在系统的操作系统名为：%@", [proInfo operatingSystemName]);
		NSLog(@"进程所在系统的操作系统版本号字符串为：%@", [proInfo operatingSystemVersionString]);
        ...
        NSLog(@"返回一个全局的，唯一的字符串：%@", [proInfo globallyUniqueString]);
